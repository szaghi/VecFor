...
type :: vector
  ...
  contains
    ...
    generic :: operator(.ortho.) => orthogonal        ! Procedure for computing the component of vec1 orthogonal to vec2.
    generic :: operator(.dist.)  => distance          ! Procedure for computing the distance between vec1 and vec2.
...
contains
  ...
  elemental function distance(vec1, vec2) result(dist)
  !< Compute the distance between vec1 and vec2.
  class(vector), intent(in) :: vec1  !< First vector.
  type(vector),  intent(in) :: vec2  !< Second vector.
  real(R_P)                 :: dist  !< Distance between the two vectors.

  dist = sqrt((vec1%x - vec2%x)**2._R_P + (vec1%y - vec2%y)**2._R_P + (vec1%z - vec2%z)**2._R_P)
